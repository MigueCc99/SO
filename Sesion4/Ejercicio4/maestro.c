...
int main(int argc, char *argv[]){
  ...
  intervalos[3] = ((intervalos[1]+intervalos[0]) / 2) - 1; // Esclavo1 fin. intervalos[4] = intervalos[3] + 1; // Esclavo2 ini.
  ...
  if(esclavo1 == 0){  // Proceso hijo1
    close(fd1[0]);  // Cierro el descriptor de lectura
    dup2(fd1[1],STDOUT_FILENO);
    if(execl("./esclavo","esclavo",ini,fin,NULL)){
      perror("\nError en el execl");
      exit(EXIT_FAILURE);
    }
    else{ //Proceso padre
  ...
  if(esclavo2 == 0){  // Proceso hijo2
    close(fd2[0]);  // Cierro el descriptor de lectura
